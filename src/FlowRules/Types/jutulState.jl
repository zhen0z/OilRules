...
function jutulSimpleState(M::jutulModel{D, T}; ρOil::T=T(ρOil), ρH2O::T=T(ρH2O), g::T=T(10.0)) where {D, T}
    ## default state at time 0 with all water
    Z = repeat((1:M.n[end])*M.d[end], inner = prod(M.n[1:2]))
    p0 = ρH2O * g * (Z .+ M.h) # rho * g * h
    state0 = setup_state(simple_model(M; ρOil=ρOil, ρH2O=ρH2O), Pressure = p0, Saturations = [0.0, 1.0])
