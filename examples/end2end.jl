...
### add box to Oil and velocity
box_Oil(x::AbstractArray{T}) where T = max.(min.(x,T(1)),T(0))
box_v(x::AbstractMatrix{T}) where T = max.(min.(x,T(3501)),T(3200))
box_v(x::AbstractVector) = [box_v(x[i]) for i = 1:length(x)]
y_init = box_Oil(O(S(T(logK_init), f)))

# objective function for inversion
fval = Inf32

function obj(logK)
    c = box_Oil(O(S(T(logK), f))); v = R(c); v_up = box_v(u(v)); dpred = F(v_up);
...
for j=1:niterations
    ...
    ### plotting
    y_predict = box_Oil(O(S(T(logK0), f)))
